...
BEGIN
  ...
  IF own_id IS NULL THEN
    ...
    INSERT INTO "Tenant" (id, name, type, notes, "advertisingEnabled", active, "createdAt", "updatedAt", "mirrorsCommercialFromId")
    VALUES (
      own_id,
      own_name,
      'RETAILER',
  ...
  ON CONFLICT ("tenantId", "userId") DO UPDATE
    SET role = 'OWNER', title = 'Superadmin de prueba', active = true, "updatedAt" = now();
END $$;
