...
import { TENANT_ROLE_LABELS, TENANT_TYPE_LABELS, type TenantRole, type TenantType } from "@nodo/shared";
...
export function assertTenantType(tenant: TenantContext, allowed: readonly TenantType[]) {
  ...
  const necesarios = allowed.map((type) => TENANT_TYPE_LABELS[type]).join(" o ");
  throw new ForbiddenException(`Esta sección es de ${necesarios}`);
}
