...
import { TENANT_ROLES_CAN_MANAGE_BRAND_DISCOUNTS, TENANT_ROLES_CAN_MANAGE_DISTRIBUTOR, TENANT_ROLES_CAN_SEE_PORTFOLIO } from "@/lib/api";
...
export function isProductManager() {
  return getTenant()?.type === "DISTRIBUTOR" && getTenant()?.role === "PRODUCT_MANAGER";
}

export function canManageBrandDiscounts() {
  const role = getTenant()?.role;
  return isDistributor() && !!role && (TENANT_ROLES_CAN_MANAGE_BRAND_DISCOUNTS as readonly string[]).includes(role);
}
