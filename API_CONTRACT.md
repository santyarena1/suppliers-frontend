...
- **Body / Params**: paginación `page`, `per_page` (máx. 100). Stock `q`, `sku`, `local_id`. Claves `{ apiKey, apiSecret, baseUrl? }` (el secret no se vuelve a devolver). Ventas `desde`, `hasta`, `estado`. Productos vendidos igual + `q`, `entrega` (estado de entrega del ítem), `sort` (`fecha|venta|cliente|producto|cantidad|precio|subtotal|estado|entrega`), `dir` (`asc|desc`). Órdenes/RMA `estado`, `cliente_id`, `q`. Escrituras (clientes, stock, ventas, compras, órdenes, cta cte, RMA) reenvían el JSON completo a AcuStock con los mismos nombres de campo que el GET/alta del sistema. No se recorta el body: un campo que aparece en Nueva Venta (tipo de operación, depósito, envío, etiquetas, Woo/ML, entrega, vencimiento, IVA, S/N, origen, percepción, etc.) se manda si viene en el POST/PATCH.
