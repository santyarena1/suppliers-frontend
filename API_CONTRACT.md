...
- **Estado**: IMPLEMENTADO
- **Notas**: La ficha (`ProviderSyncCache`) es la misma para todos los locales de ese distribuidor: nombre, marca, foto y el resto de los datos del producto. El precio, el stock y el historial son de la oferta de esa organización (`TenantProductOffer`) y salen de la cuenta con la que ese local sincronizó. Vincular no copia ofertas de otro comercio. Los importes que vengan en el JSON del proveedor no se guardan en la ficha ni se usan para mostrar el precio. Por defecto, si en ese distribuidor la config de stock 0 no es «Mostrar igual», no se listan productos con stock 0 (ni debajo del umbral). `includeOutOfStock=true` los incluye igual. La ficha individual (`GET /providers/:provider/products/:externalId`) sí los devuelve si se entra por link. Qué hacer en la sync con faltantes o stock 0 lo define cada proveedor (`missingProductAction`, `zeroStockAction`), no un comportamiento especial por marca. Marcas y categorías del catálogo son filtros generales del buscador (`/catalog/brands`, `/catalog/by-brand`), no facetas post-resultado. `/catalog/featured` son bajas de precio: un punto por día (Argentina). Sin `all`, la jornada de hoy; si hoy no hubo, la última jornada con bajas. `all=true` (Ver todas) trae las bajas de todo el historial. El gráfico de ficha (`GET .../price-history`) dibuja **un punto por cada día** desde la primera captura hasta hoy (si un día no hubo sync, copia el último precio).
...
### [FEATURE] Carrito recíproco con el portal del distribuidor (Invid, Elit, Air, New Bytes)
- **Método**: POST
...
- **Body / Params**: igual que antes (`items`, `addressId`, `paymentOption`, `deliveryOption`) más `dropPortalCodes?: string[]` (códigos que el comercio decidió sacar del carrito del distribuidor). En New Bytes también va en `POST /checkout/shipping` y `POST /checkout/preview`.
...
- **Estado**: IMPLEMENTADO
- **Notas**: No se reemplaza un carrito por el otro. La API guarda en `ProviderSyncConfig.portalCartSnapshot` la foto (`{codigo: cantidad}`) de lo ya unificado. Contra esa foto: borrado en el portal se saca de NODO, borrado en NODO se saca del portal, cantidad cambiada en el portal gana. Sin foto (primera vez, o después de confirmar), lo que está en los dos se suma una sola vez y lo que solo está en el portal queda pendiente. La foto de una suma guarda `baseQty` (la cantidad de NODO), no el total, para no volver a sumar. Lo pendiente no entra a la foto hasta que el comercio lo deje: si entrara, la pasada siguiente lo leería como borrado en NODO. `dropPortalCodes` lo saca del portal. La foto no avanza sobre cambios que el frontend todavía no reflejó. Confirmar el pedido arma solo lo que está en NODO; la UI no deja confirmar mientras haya líneas pendientes. **New Bytes**: `GET carrito` antes de `carrito/new`, también en preview y en la cotización de envío (si no, el segundo paso vaciaba el carrito unificado). **Elit**: carrito por cuenta. **Invid** y **Air**: carrito por sesión; un portal vacío no es un borrado. **Grupo Núcleo, New Tree, Solution Box, Distecna y Polytech** no tienen un carrito remoto que esta conciliación reescriba: el pedido sale del carrito de NODO. Lógica común en `portal-cart-sync.ts`; el frontend pregunta dejar/sacar en `PortalSyncNotice`.
