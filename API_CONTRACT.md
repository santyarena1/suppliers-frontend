...
- **Body / Params**: paginación `page`, `per_page` (máx. 100). Stock `q`, `sku`, `local_id`. PATCH stock `{ nombre?, precio?, stock? }`. Claves `{ apiKey, apiSecret, baseUrl? }` (el secret no se vuelve a devolver). Ventas `desde`, `hasta`, `estado`. Productos vendidos igual + `q`, `sort` (`fecha|venta|cliente|producto|cantidad|precio|subtotal|estado`), `dir` (`asc|desc`). Órdenes/RMA `estado`, `cliente_id`, `q`. Alta RMA `{ falla_reportada, producto_nombre?, producto_serie?, cliente_id?, venta_id?, venta_numero?, orden_trabajo_id? }`. El resto de escrituras (clientes, ventas, compras, órdenes, cta cte, stock alta, PATCH rma) reenvían el JSON a AcuStock con los mismos nombres de campo que el GET.
