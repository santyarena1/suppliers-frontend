...
- **Estado**: IMPLEMENTADO
- **Notas**: Un hilo es **dos personas** dentro de un `TenantLink` (`distroUserId` + `storeUserId`). Nadie ve el chat de un compañero. “Hablar” sin `peerUserId` abre con el vendedor asignado (desde el local) o el dueño/comprador del comercio (desde el distro). En pantalla: organización, nombre de usuario y rol (vendedor, PM, comprador, etc.) en ambos lados. En el comercio escriben `OWNER`/`ADMIN`/`BUYER`; el `SELLER` del local solo lee. En el distribuidor escriben `OWNER`/`ADMIN`/`SELLER`/`PRODUCT_MANAGER`. `REVOKED` no se habla; `SUSPENDED` sí. El hub SSE usa Redis (`REDIS_URL`) cuando hay más de una réplica. UI: `/mensajes`. Ver `docs/PLAN_TIPO2.md`. Tipo 3 (marca) todavía no engancha este chat.
...
- **Auth**: Bearer con organización `BRAND` (panel), `RETAILER` (marcas/avisos) o `DISTRIBUTOR`/`BRAND` (avisar a un comercio vinculado). Landing pública: sin auth. Sync: `ROLE_ADMIN`.
...
- **Respuesta esperada**: acciones `{ canWrite, actions: [{ ..., progress: { current, target, ratio, met } }] }` · landing `{ name, publicKey, publicPath, published, ... }` · marcas del comercio `{ brands: [{ name, landing, actions }] }` · sync `{ terms, created, linked, users }`
- **Estado**: IMPLEMENTADO
- **Notas**: Cada término de catálogo `BRAND` tiene Tenant + dueño placeholder (`managedByPlatform`). La medición lee `ProviderOrder` `CREATED`/`OFFLINE` de los comercios en alcance; si el ítem no trae `brand`/`displayBrand` no se cuenta. Landing `/m/:publicKey` es marketing, no catálogo. UI: `/marca`, `/marca/acciones`, `/marca/landing`, `/marca/cuentas`, `/marcas`, `/avisos`. Ver `docs/PLAN_TIPO3.md`.
