...
### [FEATURE] Tipo 3 — orgs de marca, landing, acciones y avisos
- **Método**: GET | PUT | POST
- **Ruta**: `/my/brand/landing` · `/my/brand/actions` · `/my/brand/actions/:id` · `/my/brand/actions/:id/status` · `/my/brand/accounts` · `/my/brand/notes` · `/my/brands` · `/my/notifications` · `/my/notifications/:id/read` · `/my/notifications/send` · `/public/brands/:publicKey` · `/admin/brands/sync`
- **Auth**: Bearer con organización `BRAND` (panel), `RETAILER` o `DISTRIBUTOR` (marcas/avisos; el distro como cliente de la marca), o `DISTRIBUTOR`/`BRAND` (avisar a una cuenta cliente vinculada). Landing pública: sin auth. Sync: `ROLE_ADMIN`.
- **Body / Params**: acción `{ kind: PURCHASE_QTY|PURCHASE_AMOUNT|REBATE, title, description?, startsAt, endsAt, targetQty?, targetAmountUsd?, rewardKind: NONE|FLAT|PER_UNIT, rewardUsd?, notifyRetailers?, scopes?: [{ kind: DISTRIBUTOR|RETAILER|PRODUCT, refId }] }` · estado `{ status: ACTIVE|ENDED|CANCELLED }` · landing `{ published?, headline?, about?, logoUrl?, heroUrl?, websiteUrl?, supportEmail?, supportPhone?, blocks? }` · aviso `{ retailerTenantId, title, body }` (`retailerTenantId` es el tenant cliente: comercio o distro)
- **Respuesta esperada**: acciones `{ canWrite, actions: [{ ..., progress: { current, target, ratio, met } }] }` · landing `{ name, publicKey, publicPath, published, ... }` · marcas vinculadas `{ brands: [{ name, landing, actions }] }` · cuentas `{ retailers, linkedDistributors, distributors }` · sync `{ terms, created, linked, users }`
- **Estado**: IMPLEMENTADO
- **Notas**: Cada término de catálogo `BRAND` tiene Tenant + dueño placeholder (`managedByPlatform`). La medición lee `ProviderOrder` `CREATED`/`OFFLINE` de los comercios en alcance; si el ítem no trae `brand`/`displayBrand` no se cuenta. Landing `/m/:publicKey` es marketing, no catálogo. Chat distro↔marca: el distro es `clientTenant` y habla como `storeUserId`. UI: `/marca`, `/marca/acciones`, `/marca/landing`, `/marca/cuentas`, `/marcas`, `/avisos`. Ver `docs/PLAN_TIPO3.md`.
...
- **Ruta**: `/tgs/enabled` · `/tgs/me` · `/tgs/clientes` · `/tgs/clientes/:id` · `/tgs/stock` · `/tgs/stock/:id` · `PATCH /tgs/stock/:id` · `/tgs/ventas` · `/tgs/productos-vendidos` · `/tgs/ventas/:id` · `/tgs/compras` · `/tgs/compras/:id` · `/tgs/ctacte/clientes/:id` · `/tgs/ctacte/proveedores/:id` · `/tgs/ordenes` · `/tgs/ordenes/:id` · `/tgs/rma` · `/tgs/rma/:id` · `POST /tgs/rma`
...
- **Body / Params**: paginación `page`, `per_page` (máx. 100). Stock `q`, `sku`, `local_id`. PATCH stock `{ nombre?, precio?, stock? }`. Ventas `desde`, `hasta`, `estado`. Productos vendidos igual + `q`, `sort` (`fecha|venta|cliente|producto|cantidad|precio|subtotal|estado`), `dir` (`asc|desc`). Órdenes/RMA `estado`, `cliente_id`, `q`. Alta RMA `{ falla_reportada, producto_nombre?, producto_serie?, cliente_id?, venta_id?, venta_numero?, orden_trabajo_id? }`.
- **Respuesta esperada**: listados `{ items, meta: { page, per_page, total, total_pages, local_id? } }`. Detalle = objeto AcuStock (`data` desempaquetado). Cta cte incluye `movimientos` y `meta`. `GET /tgs/productos-vendidos` → `{ items: TgsProductoVendido[], meta, ventas, truncated }` (una fila por ítem, no por comprobante).
- **Estado**: IMPLEMENTADO
- **Notas**: Proxy HTTP a `https://thegamershop.acustock.app/api/v1/sistema` con headers `X-AcuStock-Key` / `X-AcuStock-Secret` leídos de `ACUSTOCK_API_KEY` y `ACUSTOCK_API_SECRET`. El frontend nunca ve las claves. Un 401 de AcuStock se traduce a 502 para no cerrar la sesión de Nodo. Escritura de ventas/compras la API remota responde 501. UI: `/sistema-tgs`. GET stock por id numérico no existe en AcuStock (404); el detalle usa SKU. `GET /tgs/productos-vendidos` aplana `GET /ventas/:id` (ítems reales). AcuStock no manda proveedor ni etiquetas por línea: la UI muestra “Sin prov.”. Tope 250 ventas por consulta.
